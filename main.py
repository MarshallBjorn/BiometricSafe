...
def encrypt_file_operation(file_path, key, registration_template_data):
    ...
    try:
        ...
        template_file_path = base_path + extension + ".template" # E.g., file.pdf.template

        with open(secure_file_path, 'wb') as encrypted_file:
            encrypted_file.write(encrypted_content)

        with open(template_file_path, 'wb') as template_file:
            template_file.write(registration_template_data) # Save the hash of the registration image
...
def decrypt_file_operation(secure_file_path, key):
    ...
    try:
        ...
        messagebox.showerror("Decryption Error", f"Could not decrypt file: {e}. Key or file might be invalid.")
        return None
...
class BiometricSafeApp:
    def __init__(self, master_window):
        ...
        # --- Encryption Frame ---
        encryption_frame = tk.LabelFrame(master_window, text="Encrypt File", padx=10, pady=10)
        encryption_frame.pack(padx=10, pady=10, fill="x")

        tk.Label(encryption_frame, text="File to encrypt:").grid(row=0, column=0, sticky="w", pady=2)
        tk.Entry(encryption_frame, textvariable=self.file_path_to_encrypt_var, width=50).grid(row=0, column=1, pady=2)
        tk.Button(encryption_frame, text="Select File", command=self.select_file_to_encrypt).grid(row=0, column=2, padx=5, pady=2)

        tk.Label(encryption_frame, text="Fingerprint image (registration):").grid(row=1, column=0, sticky="w", pady=2)
        tk.Entry(encryption_frame, textvariable=self.fingerprint_image_encrypt_var, width=50).grid(row=1, column=1, pady=2)
        tk.Button(encryption_frame, text="Select Image", command=lambda: self.select_fingerprint_image(self.fingerprint_image_encrypt_var, "encrypt")).grid(row=1, column=2, padx=5, pady=2)
        
        self.preview_encrypt_label = tk.Label(encryption_frame) # Label for image preview
        self.preview_encrypt_label.grid(row=2, column=0, columnspan=3, pady=5)

        tk.Button(encryption_frame, text="Encrypt File", command=self.run_encryption_process, bg="lightblue", fg="black").grid(row=3, column=0, columnspan=3, pady=10)

        # --- Decryption Frame ---
        decryption_frame = tk.LabelFrame(master_window, text="Decrypt File", padx=10, pady=10)
        decryption_frame.pack(padx=10, pady=10, fill="x")

        tk.Label(decryption_frame, text="File to decrypt (.secure):").grid(row=0, column=0, sticky="w", pady=2)
        tk.Entry(decryption_frame, textvariable=self.file_path_to_decrypt_var, width=50).grid(row=0, column=1, pady=2)
        tk.Button(decryption_frame, text="Select File (.secure)", command=self.select_file_to_decrypt).grid(row=0, column=2, padx=5, pady=2)

        tk.Label(decryption_frame, text="Fingerprint image (verification):").grid(row=1, column=0, sticky="w", pady=2)
        tk.Entry(decryption_frame, textvariable=self.fingerprint_image_decrypt_var, width=50).grid(row=1, column=1, pady=2)
        tk.Button(decryption_frame, text="Select Image", command=lambda: self.select_fingerprint_image(self.fingerprint_image_decrypt_var, "decrypt")).grid(row=1, column=2, padx=5, pady=2)

        self.preview_decrypt_label = tk.Label(decryption_frame) # Label for image preview
        self.preview_decrypt_label.grid(row=2, column=0, columnspan=3, pady=5)

        tk.Button(decryption_frame, text="Decrypt File", command=self.run_decryption_process, bg="lightgreen", fg="black").grid(row=3, column=0, columnspan=3, pady=10)
        
        # Status bar
        self.status_bar = tk.Label(master_window, text="Ready.", bd=1, relief=tk.SUNKEN, anchor=tk.W)
    ...
    def select_file_to_encrypt(self):
        ...
        if path:
            ...
            self.update_status(f"Selected file for encryption: {os.path.basename(path)}")

    def select_file_to_decrypt(self):
        path = filedialog.askopenfilename(title="Select .secure file to decrypt",
                                           filetypes=(("Secure files", "*.secure"), ("All files", "*.*")))
        if path:
            self.file_path_to_decrypt_var.set(path)
            self.update_status(f"Selected file for decryption: {os.path.basename(path)}")

    def select_fingerprint_image(self, path_variable, mode):
        path = filedialog.askopenfilename(title="Select fingerprint image",
    ...
    def run_encryption_process(self):
        ...
        if not input_file or not registration_fingerprint_image:
            messagebox.showerror("Error", "Please select a file AND a fingerprint image for encryption.")
            return

        self.update_status("Generating fingerprint template (encryption)...")
        registration_template_data = simulate_generate_template_from_image(registration_fingerprint_image)
        ...
        created_secure_file, created_template_file = encrypt_file_operation(input_file, encryption_key, registration_template_data)

        if created_secure_file and created_template_file:
            messagebox.showinfo("Success", f"File encrypted as: {os.path.basename(created_secure_file)}\n"
    ...
    def run_decryption_process(self):
        ...
        if not input_secure_file or not verification_fingerprint_image:
            messagebox.showerror("Error", "Please select a .secure file AND a fingerprint image for decryption.")
            return
        ...
        if not os.path.exists(stored_template_file_path):
            messagebox.showerror("Error", f"Template file not found: {os.path.basename(stored_template_file_path)}. "
        ...
        verification_template_data = simulate_generate_template_from_image(verification_fingerprint_image)
        ...
        templates_match = simulate_compare_templates(registered_template_data, verification_template_data)
        ...
        if templates_match:
            ...
            self.update_status(f"Decrypting file: {os.path.basename(input_secure_file)}...")
            decrypted_file = decrypt_file_operation(input_secure_file, decryption_key)
            if decrypted_file:
                messagebox.showinfo("Success", f"File decrypted successfully as: {os.path.basename(decrypted_file)}")
                self.update_status("Decryption completed successfully.")
            else:
                self.update_status("Decryption failed.")
        else:
            messagebox.showerror("Access Denied", "Fingerprints do not match. Cannot decrypt file.")
